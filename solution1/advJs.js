...
//code
const dSizeArr = data.map((el) => el.defender_size).filter(el => el != null)
...
const b_type = data.map((el) => el.battle_type).filter(el => el != "" || null )  //removing null elements (will effect on average).

const repeat_max = (arr) =>
  arr.reduce((acc,curr,index,arr)=>(arr.filter(x => x === acc).length >= arr.filter(x => x === curr).length ? acc : curr), null)

const at_king = data.map((el) => el.attacker_king)
const df_king = data.map((el) => el.defender_king)
...
const answer = {
  'most active':{
    'attacker_king':repeat_max(at_king),
    'defender_king':repeat_max(df_king),
    'region':repeat_max(region),
    'name':repeat_max(name)
  },
  'attacker_outcome':{
    'win':data.filter(el => el.attacker_outcome == "win").length,
    'loss':data.filter(el => el.attacker_outcome == "loss").length
  },
  'battle_type':[...new Set(b_type)],
  'defender_size':{
    'average':calcAverage(dSizeArr),
    'min':Math.min(...dSizeArr),
    'max':Math.max(...dSizeArr)
  }
}
